...
(defcustom mouse-wheel-progressive-factor '(1 ((double) . 1) ((triple) . 1))
...
less than a full screen."
  ...
  :type '(cons
	  (choice :tag "Normal"
		  ...
		  (cons
		   (repeat (choice :tag "factor" (const double) (const triple)))
		   (choice :tag "progressive factor"
			   (integer :tag "Multiplied by scroll amount"))))
          (repeat
           (cons
            (repeat (choice :tag "factor"
			    (const double) (const triple)))
            (choice :tag "progressive factor"
                    (integer :tag "Multiplied by scroll amount")))))
  :set 'mouse-wheel-change-button)
...
(defun mwheel-acceleration (event)
  (let* ((mods (delq 'super (delq 'shift (delq 'meta (delq 'alt (delq 'control (delq 'hyper (event-modifiers event))))))))
         (amt (assoc mods mouse-wheel-progressive-factor)))
    (if amt (setq amt (cdr amt))
      (let ((list-elt mouse-wheel-progressive-factor))
        (while (consp (setq amt (pop list-elt))))))
    ;; (message "accelleration factor: %d" amt)
    (or amt 1)))
...
systems."
  ...
  (let* ((curwin (if mouse-wheel-follow-mouse
    ...
    (when (and mouse-wheel-progressive-speed (numberp amt))
      ...
      (setq amt (* amt (mwheel-acceleration event))))
    (unwind-protect
